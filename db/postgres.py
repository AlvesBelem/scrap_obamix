...
PRICE_VENDA_MULTIPLIER = Decimal("2.56")
...
def connect_db(config: Dict[str, Any], attempt_create: bool = True):
    ...
    """
    auto_create = config.get("auto_create_db", True)
    try:
        return psycopg2.connect(**_connection_kwargs(config))
        ...
        if not auto_create or not attempt_create or not db_missing:
            raise
...
def _create_database(config: Dict[str, Any]) -> None:
    ...
    connection = psycopg2.connect(**_connection_kwargs(config, db_override=maintenance_db))
...
def _inject_price_venda(df_products: pd.DataFrame) -> pd.DataFrame:
    ...
    def calc_price_venda(value):
        ...
        multiplied = (decimal_value * PRICE_VENDA_MULTIPLIER).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        integer_part = multiplied.to_integral_value(rounding=ROUND_DOWN)
        fractional = multiplied - integer_part

        if fractional > HALF_DECIMAL:
            base = integer_part
        else:
            base = integer_part - Decimal("1")
            if base < Decimal("0"):
                base = Decimal("0")

        return base + PRICE_SUFFIX
...
def _connection_kwargs(config: Dict[str, Any], db_override: str | None = None) -> Dict[str, Any]:
    target_db = db_override or config.get("dbname")
    if not target_db:
        raise ValueError("Database name must be provided.")

    params: Dict[str, Any] = {"dbname": target_db}
    for key in ("user", "password", "host", "port"):
        value = config.get(key)
        if value:
            params[key] = value

    for key in OPTIONAL_CONN_KEYS:
        value = config.get(key)
        if value:
            params[key] = value
